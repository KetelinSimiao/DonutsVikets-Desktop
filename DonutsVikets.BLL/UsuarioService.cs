...
namespace Project.BLL.Services
{
    ...
    public class UsuarioService
    {
        ...
        public async Task<(bool success, string message)> CreateAsync(UsuarioDTO dto)
        {
            ...
            var existing = await _repo.GetByEmailAsync(emailNormalizado);
        ...
        public async Task<Usuario?> LoginAsync(string email, string senhaDigitada)
        {
            var usuario = await _repo.GetByEmailAsync(email);
            if (usuario == null)
                throw new Exception("Usuário não encontrado.");

            var senhaHash = HashPassword(senhaDigitada);
